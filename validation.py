### FIRST ATTEMPT AT VALIDATION. GENERATE TABLE SETTING OUT INITIAL SCENAIRO BASED ON DEFAULT VALUES IN CONFIG.PY. THEN RANDOMLY SELECT A VARIABLE, UPDATE IT RANDOMLY, AND PRINT OUT THE NEWLY CALCULATED SCENARIO. THEN ANOTHER RANDOMLY SELECTED VARIABLE, UPDATED RANDOMLY. THE PURPOSE IS TO GENERATE TABLES THAT CAN BE EYEBALLED TO IDENTIFY POTENTIAL ISSUES. THIS IS, OF COURSE, NOT THE ONLY WAY (AND PROBABLY ISN'T THE BEST WAY!). ALSO, THIS CODE IS NOT GREAT AT THE MOMENT (NEED TO BE BETTER WITH HOW VARIABLES ARE UPDATED, AND ALSO GENERATES REDUNDANT TABLES.)
...
def modify_variable(var_name, original_value):
    ...
    if isinstance(original_value, bool):
        ...
        return (
            new_value if new_value != 0 else original_value + 1
        )  # Ensure new_value is not zero
    elif isinstance(original_value, float):
        new_value = original_value * random.uniform(0.5, 1.5)
        return (
            new_value if new_value != 0 else original_value * 1.1
        )  # Ensure new_value is not zero
