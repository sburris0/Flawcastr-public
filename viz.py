...
init_plot_widget_stylesheet_background_color = (
    "#F7F5F2"  # Very light beige (same as Plot Widget Facecolor)
)
...
class MyWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setStyleSheet(
            f"QWidget {{ background-color: {qwidget_background_colour}; }}"
        )
        self.splitter.setStyleSheet(
            f"QSplitter::handle {{ background-color: {qsplitter_background_colour}; }}"
        )
        ...
        self.splitter.addWidget(
            scroll
        )  # Add the scroll area to the splitter instead of self.input_widget
        ...
        self.plot_needs_update.connect(
            self.update_plot
        )  # Connect the signal to update the plot
    ...
    def plot_deterministic_balances(self, years):
        try:
            ...
            if len(years) != len(deterministic_balances):
                adjusted_years = (
                    np.arange(len(deterministic_balances)) + config.client1_age
                )
            else:
                adjusted_years = years + config.client1_age
    ...
    def plot_saved_scenarios(self, years):
        ...
        if self.validation_labels:
            ...
            for name, scenario in self.validation_labels.items():
                ...
                self.ax.plot(
                    adjusted_years, scenario, color="pink", linewidth=2, alpha=0.7
                )
    ...
    def plot_probabilistic_balances(self, years=None):
        ...
        try:
            ...
            self.ax.text(
                0.5,
                0.5,
                "Error: Unexpected plotting error",
                transform=self.ax.transAxes,
                ha="center",
                va="center",
            )
    ...
    def set_plot_aesthetics(self):
        ...
        max_vertical_limit = (
            2 * self.max_deterministic_balance
        ...
        self.ax.grid(
            True, which="both", linestyle="--", linewidth=0.25, color="gray", alpha=0.5
        )
